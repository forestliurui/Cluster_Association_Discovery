function [result_4method,MI_conf,MI_author,MI_matrix]=DBLP_labeled_feature_test_function(intensity_noise)
...
F_t_c(:,W_index_remove_conf)=[];
conf_ID(W_index_remove_conf)=[];
conf_label(W_index_remove_conf)=[];
...
%add noise to association matrix
% A2=A2+max(max(A2))*intensity_noise*rand(size(A2));
...
MI_conf(iteration_index)=mutual_information_metric(conf_class_predicted4MI,conf_class_truth4MI);
MI_author(iteration_index)=mutual_information_metric(author_class_predicted4MI,author_class_truth4MI);

MI_matrix(iteration_index)=mutual_information_matrix(W_a_c,ind_H1,ind_H2);
fprintf(1,'MI of W: %f\n',MI_matrix(iteration_index));
...
result_4method(iteration_index)=F_score;
end