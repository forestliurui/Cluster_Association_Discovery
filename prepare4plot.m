...
load('SNP_gene_assoc_minus_log_matrix_inf_to_0.mat');
...
[PPI_x, PPI_y]=textread('01PPINetworkEdges.txt');

A_PPI=sparse(PPI_x+1,PPI_y+1,ones(length(PPI_x),1),size(W,2),size(W,2));
...
index_gene_all=union(index_gene_all, index_Gene);
...
A=A_PPI(index_gene_all, index_gene_all);
